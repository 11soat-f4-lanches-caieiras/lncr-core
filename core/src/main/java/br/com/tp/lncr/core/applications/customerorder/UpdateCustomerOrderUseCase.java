...
public class UpdateCustomerOrderUseCase {
    ...
    public CustomerOrder updateStatusById(Integer customerOrderId, String newStatus, Boolean forceUpdate) {
        LoggerUtil.info("Iniciando atualização de status do pedido com id: " + customerOrderId + " para o status: " + newStatus);
        ...
        if (updateCustomerOrder != null) {
            ...
            LoggerUtil.info("Status do pedido atualizado com sucesso: " + updateCustomerOrder.getId() + " para o status: " + updateCustomerOrder.getStatus());
    ...
    private void updateStatusToCancel(CustomerOrder updateCustomerOrder, String oldStatus) {
        LoggerUtil.info("Atualizando status do pedido: " + updateCustomerOrder.getId() + " para Cancelled");
        try {
            this.customerOrderGateway.cancelPaymentChargeByCustomerOrderId(updateCustomerOrder.getId());
        } catch (Exception e) {
            LoggerUtil.error("Erro ao cancelar cobrança do pedido: " + updateCustomerOrder.getId());
        ...
        if (oldStatus.equals(CustomerOrderStatus.RECEIVED.getDescription())) {
            ...
            if (kitchenOrderDTO != null) {
                try {
                    ...
                    LoggerUtil.error("Erro ao cancelar pedido na cozinha: " + kitchenOrderDTO.getId());
                    LoggerUtil.error(e.getMessage());
                }
            }
        }
    ...
    private void updateStatusToReceived(CustomerOrder updateCustomerOrder) {
        LoggerUtil.debug("Atualizando status do pedido: " + updateCustomerOrder.getId() + " para Received");
